<ul class="rounded-[4px] z-[999] bg-white box-shawdow-dropdown p-4 pr-0 transform scale-0 group-hover:scale-100 absolute transition duration-150 ease-in-out origin-top min-w-[219px]">
        ...
        <li class="flex rounded-sm py-2 px-1">
            @if(count($item->children) > 0)
                <button class="w-full text-left flex items-center outline-none focus:outline-none justify-between">
                    <a href="{{ $item->url }}"
                       class="pr-1 text-[#363F4E] font-[600] text-[16px] leading-6 line-clamp-1">{{ $item->name }}</a>
            ...
            @else
                <a href="{{ $item->url }}"
                   class="w-full text-[#363F4E] font-[600] text-[16px] leading-6  line-clamp-1">{{$item->name}}</a>
            @endif
        </li>
    @endforeach
</ul>