<nav id="sidebar-menu" x-description="Mobile menu" x-bind:aria-expanded="open"
     ...
     class="fixed w-64 transition-all duration-500 ease-in-out h-screen bg-white shadow-sm z-[11]">
    <div class="h-full overflow-y-auto scrollbars">
        ...
        <!-- Sidebar menu -->
        <ul id="side-menu" x-data="{selected:'manage-tab'}"
            class="w-full float-none flex flex-col font-medium ltr:pl-1.5 rtl:pr-1.5">
            <!-- dropdown -->
            <li class="relative">
                <a :class="{ 'text-indigo-500': selected == 1 }"
                   @click="selected !== 1 ? selected = 1 : selected = null"
                   class="block py-2.5 px-6 hover:text-indigo-500" href="javascript:;">
                ...
                <ul x-show="selected == 1" x-transition:enter="transition-all duration-200 ease-out"
                    ...
                    <li class="relative">
                        <a class="text-indigo-500 block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500"
            ...
            <li class="relative">
                <a :class="{ 'text-indigo-500': selected == 'manage-tab' }"
                   @click="selected !== 'manage-tab' ? selected = 'manage-tab' : selected = null"
                   class="block py-2.5 px-6 hover:text-indigo-500" href="javascript:;">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                         class="inline-block h-4 w-4 ltr:mr-2 rtl:ml-2 bi bi-house-door" viewBox="0 0 16 16">
                        <path
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                     class="transform transition duration-300 mt-1.5 bi bi-chevron-down"
                     :class="{ 'rotate-0': selected == 'manage-tab', 'ltr:-rotate-90 rtl:rotate-90': !(selected == 'manage-tab') }"
                     width=".8rem" height=".8rem" viewBox="0 0 16 16">
                  <path fill-rule="evenodd"
                        d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"/>
                </svg>
                ...
                <!-- dropdown menu -->
                <ul x-show="selected == 'manage-tab'" x-transition:enter="transition-all duration-200 ease-out"
                    x-transition:enter-start="transform opacity-0 scale-95"
                    x-transition:enter-end="transform opacity-100 scale-100"
                    class="block rounded rounded-t-none top-full z-50 ltr:pl-7 rtl:pr-7 py-0.5 ltr:text-left rtl:text-right mb-1 font-normal">
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(3), ['artist']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.partner.artist') }}">Artist</a>
                    </li>

                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(3), ['singer']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.partner.singer') }}">Singer</a>
                    </li>

                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['topic']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.topic') }}">Topic</a>
                    </li>

                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['category']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.category') }}">Category</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['tag']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.tag') }}">Tag</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['news']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.news') }}">News</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['song']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.song') }}">Song</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['leader']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.leader') }}">Leader</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['menu']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.menu') }}">Menu</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['slide']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.slide') }}">Slides</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['top-partner']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.top-partner') }}">Top Partner</a>
                    </li>
                    <li class="relative">
                        <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 @if(in_array(Request::segment(2), ['album']))
                                {{ 'text-indigo-500' }}
                            @endif"
                           href="{{ route('admin.album') }}">Albums</a>
                    </li>
