        ...
        <div class="mx-auto p-2">
            ...
            <div class="flex flex-wrap flex-row">
                <div class="flex-shrink max-w-full px-4 w-full">
                    <p class="text-xl font-bold mt-3 mb-5 capitalize">{{ $type }} list
                    </p>
                </div>
                <div class="flex-shrink max-w-full px-4 w-full mb-6">
                    <div class="p-8 mb-6 bg-white rounded-lg shadow-lg">
                        <div class="flex justify-between mb-3">
                            <a href="{{ route('admin.partner.' . $type . '.add') }}"
                               class="flex items-center h-[38px] capitalize py-2 px-4 text-center mb-3 rounded leading-5 text-gray-100 bg-indigo-500 border border-indigo-500 hover:text-white hover:bg-indigo-600 hover:ring-0 hover:border-indigo-600 focus:bg-indigo-600 focus:border-indigo-600 focus:outline-none focus:ring-0">
                                ...
                                <table
                                    ...
                                    @foreach ($items as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->story }}</td>
                                            <td>{{ $item->prize }}</td>
                                            <td>{{ $item->featured_song }}</td>
                                            <td>{{ $item->instrumental_music }}</td>
